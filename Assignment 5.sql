...
create table Movie(mID int primary key, title varchar(50), year int check(year >= 1900), director varchar(50), check((director = 'Steven Spielberg' and year <= 1990) or (director = 'James Cameron' and year >= 1990)));
create table Reviewer(rID int primary key, name varchar(50) not null);
create table Rating(rID int null, mID int null, stars int not null check(stars between 1 and 5), ratingDate DATE null check(ratingDate > '1999-12-31'));
go 
